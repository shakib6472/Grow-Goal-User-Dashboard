<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.6.0/css/all.min.css"
    integrity="sha512-Kc323vGBEqzTmouAECnVceyQqyqdsSiqLQISBL29aUW4U/M7pSPA/gEUZQqv1cwx4OnYxTxve5UMg5GT6L4JJg=="
    crossorigin="anonymous" referrerpolicy="no-referrer" />
<!-- Google font  -->
<script src='https://kit.fontawesome.com/46882cce5e.js' crossorigin='anonymous'></script>
	<link rel="preconnect" href="https://fonts.googleapis.com">
	<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
	<link href="https://fonts.googleapis.com/css2?family=Podkova:wght@400..800&display=swap" rel="stylesheet">
	<link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Jost:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">



	<div class="main-custom-page">
        <div class="custom-nav-bar custom-background">
			<div class="custom-nav-child-img">
				<div class="custom-child-images active">
					<img src="https://growgoal.online/wp-content/uploads/2024/09/book.png" alt="">
					<h3 class="custom-lesson-name">Book A Lesson</h3>
				</div>
				<div class="custom-child-images">
					<img src="https://growgoal.online/wp-content/uploads/2024/09/book.png" alt="">
					<h3 class="custom-lesson-name">Enter The Lesson</h3>
				</div>
				<div class="custom-child-images">
					<img src="https://growgoal.online/wp-content/uploads/2024/09/book.png" alt="">
					<h3 class="custom-lesson-name">Practice</h3>
				</div>
				<div class="custom-child-images">
					<img src="https://growgoal.online/wp-content/uploads/2024/09/book.png" alt="">
					<h3 class="custom-lesson-name">Messages</h3>
				</div>
				<div class="custom-child-images">
					<img src="https://growgoal.online/wp-content/uploads/2024/09/book.png" alt="">
					<h3 class="custom-lesson-name">Money Tools</h3>
				</div>
			</div>
			
			
			<div class="custom-nav-child-profile">
				<div class="custom-profile-one">
                    <i class="fas fa-bell custom-bell" ></i>
					 <img src="https://growgoal.online/wp-content/uploads/2024/09/image-1.png" class="custom-porfile" alt="">
				</div>
				<div class="custom-profile-two">
					<h2 class="custom-student-name">Tynisha Obey</h2>
					<h4 class="custom-student">Student</h4>
				</div>

			</div>
			<div class="custom-nav-child-logout">
				<div class="custom-logout-button-container">
					<button class="custom-logout-button">
						Logout
						<i class="fas fa-right-long"></i>
					</button>
				</div>
				
			</div>
		</div>
		
		

		<!-- Second Container page 1 -->
		<div class="custom-pageone-sec-container">
			<div class="custom-page-two">
				<!-- ----Left Container -->
			<div class="custom-sec-left-container">
				<div class="custom-sec-clock-box border">
					<div class="sec-clock-img-clock-container">
						<img src="https://growgoal.online/wp-content/uploads/2024/09/Mask-group-6.png" alt="" class="sec-clock-img">
					    <h4 class="sec-clock">Clock :</h4>
						
					</div>
					<h2 class="custom-sec-current-time">8:00AM</h2>
					<h2 class="custom-sec-current-date">Monday,3rd October,2022</h2>
				</div>
				<div class=" custom-six-roting-box border">
					<div class="sec-clock-img-clock-container">
						<img src="https://growgoal.online/wp-content/uploads/2024/09/Group-40.png" alt="" class="sec-clock-img">
					    <h4 class="sec-clock">Daily Reminder:</h4>
						
					</div>
					<h4 class="custom-sec-roting-heading">(here the student will see the messages from the teacher also the assignments)</h4>
                    
				</div>
				<div >
					
				 <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883289.png" alt="" class="cutom-sec-right-arrow">
				</div>
			</div>
			<!-- -------Right Container------- -->
			<div class="six-todo-container border">
				<h2 class="custom-six-right-main-heading">The more you question, the<br> more you learn</h2>
                <h2 class="custom-six-question-form">send a message or a question to your teacher</h2>
                <div class="custom-six-contact-form">
                    <input type="text" name="" id="" placeholder="Text Here">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Vector-9.png" alt="">
                </div>
			    <h2 class="custom-six-question-form">choose your Student</h2>
                <div class="custom-teacher-choose-min-container">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883381-1.png" alt="">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883380.png" alt="">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883379.png" alt="">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883378.png" alt="">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883381-1.png" alt="">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883381-1.png" alt="">
                    <img src="https://growgoal.online/wp-content/uploads/2024/09/Group-1597883375.png" alt="">
                </div>
			</div>
			</div>
		</div>
	</div>